...
<script>
	...
	export default {
		...
		methods: {
			toChecking2(){
				if(!(this.user.checkedScore>0)){return}
				uni.navigateTo({
					url: '/pages/user/wd-list'    //wd
				})
			},
			toChecking(){
				if(!(this.user.applyScore>0)){return}
				uni.navigateTo({
					url: '/pages/user/wd-list'   //wd
				})
			},
			...
			withdraw(){
            	let b1=this.user.readyScore>0; //可提现金额大于0
