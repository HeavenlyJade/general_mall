				...
				<block v-for="(item, index) in cartList" :key="item.id">
					...
					>
						<view class="image-wrapper" style="display: block; opacity: 1;">
							<image style="width: 6.5rem;height: 6.5rem;" 
						...
						<view class="item-right">
							<view class=" title">{{$endwith(item.goods.name,20)}}</view>
	...
	export default {
		...
		methods: {
						...
						for(let i in li){
							...
							let cartItem = {
								...
								goods: {
									...
									image: item.images[0],
									type: 1, // 默认类型
									postageConfig: null // 如需运费配置，请根据实际数据结构调整
								},
								sku: {
									id: item.stock.id,
									sku: item.product_spec ,
									price: item.price,
			...
			deleteCartItem(index){
				...
				// 使用新的API删除购物车商品
				this.$post("/api/v1/wx_mini_app/shop-cart/delete", { 
					id: id 
				}, (res) => {
					if(res.code === 200) {
